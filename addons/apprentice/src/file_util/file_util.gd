...
static func is_image(file_path: String):
	var file = FileAccess.open(file_path, FileAccess.READ)
	if file:
		var bytes = file.get_buffer(mini(file.get_length(), 256))
		var file_type = FileType.get_type(bytes)
		return file_type in ["png", "jpeg", "jpg", "bmp", "svg", "webp", "ico", "icon"]
	return false
...
static func load_image(file_path: String) -> Image:
	...
	match file_path.get_extension().to_lower():
		"svg":
			...
			return image
		"ico", "icon":
			var images : Array = IcoUtil.load_ico(file_path)
			if images.size() > 0:
				return images[0]
			return null
		"icns":
			var images : Array = IcnsUtil.load_icns(file_path)
			if images.size() > 0:
				return images[0]
			return null
...
## 将图片保存到磁盘中
static func save_image(image: Image, path: String) -> Error:
	var err: Error
	match path.get_extension().to_lower():
		"png": err = image.save_png(path)
		"jpg", "jpeg": err = image.save_jpg(path)
		"webp": err = image.save_webp(path)
		"exr": err = image.save_exr(path)
		"dds": err = image.save_dds(path)
		"ico", "icon": 
			err = OK if IcoUtil.save_ico_single(image, path) else FAILED
		"icns": 
			err = OK if IcnsUtil.save_icns_single(image, path) else FAILED
		_: 
			err = FAILED
			printerr("不支持这个文件类型：", path.get_extension().to_lower())
	return err

static func load_image_by_buffer(body: PackedByteArray) -> Image:
	var file_type : String = FileType.get_type(body)
	var error : Error
	if file_type:
		var image := Image.new()
		...
		elif file_type.contains("svg"):
			image.load_svg_from_buffer(body)
		elif file_type.contains("jpeg") or file_type.contains("jpg"):
			error = image.load_jpg_from_buffer(body)
		elif file_type.contains("bmp"):
			error = image.load_bmp_from_buffer(body)
		elif file_type.contains("ico"):
			image = load_ico_from_buffer(body)
		elif file_type.contains("dds"):
			error = image.load_dds_from_buffer(body)
		elif file_type.contains("ktx"):
...
static func read_as_json(
	...
	var json = read_as_string(file_path)
	if json != null:
...
## 创建新的文件名。不会与其他文件名重复
static func new_file_path(file_path: String) -> String:
	var origin_dir = file_path.get_base_dir()
	var origin_file_name = file_path.get_file().get_basename()
	var origin_file_suffix = file_path.get_file().get_extension()
	var regex: RegEx
	...
	while true:
		if FileUtil.file_exists(file_path):
			if regex == null:
				regex = RegEx.new()
				regex.compile(".*?(\\d+)\\.*$")
			result = regex.search(file_path)
			if result:
				last_idx = int(result.get_string(1)) + 1
			else:
				last_idx += 1
			file_path = origin_dir.path_join("%s_%d.%s" % [origin_file_name, last_idx, origin_file_suffix])
		else:
			break
...
## 获取包数据
static func get_packed_scene_data(scene: PackedScene) -> Dictionary:
	if not scene:
		return {}
	var state : SceneState = scene.get_state()
	var property: StringName
	var value: Variant
	var node_path_dict: Dictionary = {}  #路径对应的节点数据
	for i in state.get_node_count():
		var node_data : Dictionary = {}
		node_data["type"] = state.get_node_type(i)
		node_data["name"] = state.get_node_name(i)
		var node_property_dict: Dictionary = {}
		for p in state.get_node_property_count(i):
			property = state.get_node_property_name(i, p)
			value = state.get_node_property_value(i, p)
			node_property_dict[property] = value
		node_data["propertys"] = node_property_dict
		node_data["is_packed_scene"] = state.is_node_instance_placeholder(i)
		if node_data["is_packed_scene"]:
			var node_packed_scene : PackedScene = state.get_node_instance(i)
			node_data["packed_scene"] = get_packed_scene_data(node_packed_scene)
		node_path_dict[state.get_node_path(i)] = node_data
